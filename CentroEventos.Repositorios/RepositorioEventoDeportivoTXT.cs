...
public class RepositorioEventoDeportivoTXT: IRepositorioEventoDeportivo
{
    ...
    public List<EventoDeportivo> ListarEventoDeportivo(){//MODIFICADO POR LA BAJA LOGICA
        ...
        while(!sr.EndOfStream){// mientras no termine el archivo.
            ...
            if(IdLinea.StartsWith("*")){//aca verifico que no empiece por "*", osea que no tenga borrado logico.
                for(int i=0;i<6;i++){
                    sr.ReadLine();//si tiene borrado logico, lo salto leyendo las demas lineas.
                }
            }
            else{// si no leo las linea y las agrego a la lista.
    ...
    public void EliminarEventoDeportivo(int Id){//CONSIDERAR QUE SI HAGO UN BORRADO LOGICO TENGO QUE CAMBIAR EL LISTAR.
        ...
        while(!sr.EndOfStream){                            // un "*" al evento a borrar(borrado logico).
            ...
            if( !l[0].StartsWith("*") && int.Parse(l[0].ToString())==Id){// verifico si es el id que busco, pero antes verifico que ya no este borrado.
